...
def get_neolix_list(root, label_file):
    ...
    for line in label_content:
        if "url" in line:
            continue
        content = line.split()
        _, img_name, str_result = content
        result = json.loads(str_result)
        names.append("images/" + img_name)
        label = []
        # pdb.set_trace()
        for lane in result["result"][0]["elements"]: 
            if "drive_line" not in lane["attribute"].keys() or lane["markType"]=="area" or lane["attribute"][ "drive_line" ] == "road_edge":
                continue
...
if __name__ == "__main__":
    ...
    # training set
    names,line_txt = get_neolix_list(args.root, "label.txt")
